...
plotCrossCorPeakEvolution = True
plotSNR = True
...
for zpadCoeff in zpadCoeffs:
    ...
    if zpadCoeff==0:
        ...
        zpad_str = '_zPadCoeff'+str(int(zpadCoeff))+'p'+str(int((zpadCoeff % int(zpadCoeff))*10))
    ...
    ax1.xaxis.set_ticks( data[0][1:][::2] )
    if plotCrossCorPeakEvolution: ax2.plot( data[0] , data[2] , label=zpad_str_forGraph , color=colors[zpadCoeff] , marker='.')
